...
class MedCLIPSearcher:
    def __init__(self, img_dir: str = "Dataset/images/"):
        ...
        # Initialize MedCLIP with fallback strategy
        ...
        try:
            ...
            print("⚠️ Using model without pretrained weights (random initialization)")
...
def create_test_queries(test_data: List[Dict]) -> List[Tuple[str, List[str]]]:
    """
    ...
    for desc, img_paths in desc_to_images.items():
        if len(img_paths) >= 2:  # At least 2 images with same description
...
def main():
    # Configuration
    IMG_DIR = "/kaggle/input/ent-data-424/processed/test"
    JSON_PATH = "/kaggle/input/ent-data-424/processed/splits_info.json"
    K_VALUES = [1, 3, 5, 10]
    MAX_SAMPLES = 300  # Limit for testing
    
    print("="*60)
    print("MedCLIP Baseline Evaluation")
    print("="*60)
    ...
    test_queries = create_test_queries(indexed_data)
    ...
    # Save search results
    results_file = "search_results_medclip_baseline.json"
    ...
    try:
        ...
        # Print results
        print("\n" + "="*60)
        print("MEDCLIP BASELINE RESULTS")
        print("="*60)
        ...
        # Save metrics
        metrics_file = "medclip_baseline_metrics.txt"
